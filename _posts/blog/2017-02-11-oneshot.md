...
\\[ S =  \{ (x_c) \}_{c=1}^{N} \\]
 ...
 It is also given \\( x\hat{} \\), the test example it has to classify. \\( x\hat{} \\) and \\(x_c \\) are all vectors \\( \in  \mathbb{R}^n \\) for some n.
...
(Note that this isn't the only way to learn a non-parametric classifier; [Lillicrap et al, Matching Networks for One-Shot Learning]() learns a complete nearest neighbours classifier end-to-end, not just a metric, comparing the test example with the entire support set, this will be discussed more later.
...
##Paragraphs I want to include but don't know where to put
