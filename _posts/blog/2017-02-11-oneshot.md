...
<figure>
	...
	<figcaption>I originally planned to have [craniopagus twins](https://en.wikipedia.org/wiki/Craniopagus_twins) as the accompanying image for this section but ultimately decided that siamese cats would go over better..</figcaption>
</figure>
...
However, the siamese network needs examples of both same and different class pairs. There are \\( E\\) examples per class, so there will be  \\( {\binom {E}{2}}  \\) pairs for every class, which means there are  \\(N_{same} = {\binom {E}{2}}C  \\) possible pairs with the same class -  183,160 pairs for omniglot. Even though 183,160 example pairs is plenty, it's only a thousandth of the possible pairs. This is important because the siamese network should be given a 1:1 ratio of same-class and different-class pairs to train on.
...
[This repository has an ipython notebook where you can just train the network.](https://github.com/sorenbouma/keras-oneshot)
...
We've just trained a neural network trained to do same-different pairwise classification on symbols. More importantly, we've shown that it can then get 85% accuracy in 20 way one-shot learning on symbols from other alphabets. But why does this work?
