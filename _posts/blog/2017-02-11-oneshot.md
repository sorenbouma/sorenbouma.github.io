...
One cool thing I noticed about training on pairs is that there are combinatorialy many pairs of images to train the model on, making it hard to overfit. Say we have \\( E\\) examples each of \\( E\\) classes. Since there are \\( C*E \\) images total, the total number of possible pairs is given by
\\[N_{pairs} = {\binom {C*E}{2}} = {\frac {(C*E)!}{2!(C*E-2)!}}  ]\\

For omniglot with its 20 examples of 964 training classes, this leads to 185,849,560 possible pairs, which is huge!
However, the siamese network needs examples of both same and different class pairs. There are \\( E\\) examples per class, so there will be  \\( {\binom {E}{2}}  \\) pairs for every class, which means there are  \\(N_{same} = C{\binom {E}{2}}  \\) possible pairs with the same class -  183,160 pairs for omniglot. Only a thousandth of the pairs have the same class, which is important because the siamese network should be given the same number of same-class and different-class pairs at each iteration.
