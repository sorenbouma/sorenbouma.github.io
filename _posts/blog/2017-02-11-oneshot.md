...
\\[ C(x\hat{}) = \underset{c}{\operatorname{argmin}}  \\]
...
#..now use the L1 distance as an input to a fully connected layer with a single output and

#sigmoid to squash the output into [0,1] and make it a probabilty. Compile with binary cross

#entropy as the loss and the same optimizer as the paper.
