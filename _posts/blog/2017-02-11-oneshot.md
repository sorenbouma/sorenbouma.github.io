...
Now we define the convnet, using the same architecure as the original paper. Since the siamese network legs share weights, we only need to define it once and then call it with each legs input layer.
...
Keras allows you to merge layers together with a lambda(one line function)
...
..now use the L1 distance as an input to a fully connected layer with a single output and sigmoid to turn it into a probability and use binary crossentropy as the loss function.  
