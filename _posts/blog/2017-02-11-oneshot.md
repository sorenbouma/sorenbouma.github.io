...
As you can see, it performs worse on tasks from the validaiton set than the train set, especially for high values of N, so there must be overfitting. It would be interesting to see how well traditional regularization like dropout works when the validation set is made of completely different classes to the training set.
...
[One-shot Imitation learning](https://arxiv.org/abs/1703.07326) is one of my favourite o.s.l
...
1. Having a neural net that from the current state and a sequence of states(the human demonstration) to an action
