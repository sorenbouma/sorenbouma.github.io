...
\\[ S =  \{ \{ x_c \}_{c=1}^{N} \\]
 ...
 It is also given \\( \hat{x} \\), the test example it has to classify. \\( \hat{x} \\) and \\(x_c \\) are all vectors \\( \in  \mathbb{R}^n \\) for some n. There are fancier ways of defining the provlem, but this one is ours.
...
#build convolutional network to use in each siamese twin
convnet = Sequential()
...
#get convnet outputs for each input image
encoded_l = convnet(left_input)
encoded_r = convnet(right_input)
#compute the l1 distance betwen the convnets output for each image
...
Koch et al uses a fixed training set of 150,000 pairs. Since that won't fit in my computers memory, I decided to just randomly sample pairs.
...
Once the learning curve flattened out, I used the weights which got the best validation 20 way accuracy for testing. Here is a plot of how the accuracy varied across one-shot tasks with different numbers of possible classes. Each data point was the average of 300 random one-shot tasks. My network got ~83% accuracy for tasks from the validation set, compared to 93% in the original paper. I'm guessing this difference is due to 
...
As I touched on earlier, I think a major flaw of this siamese approach is that it only compares the test image to every support image individualy, when it should be compariyng it to the support set as a whole. For any one shot task, \\( p( \hat{x} \circ x_2) \\) is the same no matter what else is the support set. This is silly. Say you're doing a one-shot task and you see an image that looks similar to the test image. You should be much less confident they have the same class if there is another image in the support set that also looks similar to the test image. [Matching Networks for One Shot learning](https://arxiv.org/pdf/1606.04080.pdf) uses a deep model to compare the test image to the entire support set. Embedding each example with a convnet and then a bidirectional LSTM.


### What next? / Conclusion

The omniglot dataset has been around since 2015, and already ML algorithms are getting within the ballpark of human level performance on certain one-shot learning tasks. Hopefully one day it will be seen as a mere "sanity check" for one-shot classification algorithms much like MNIST is for supervised learning.
...
Ideas from one-shot learning could be used for more sample efficient reinforcement learning, especially for things like OpenAI's Universe, where there are lots of MDPs/environments that have similar visual features and dynamics. - It would be cool to have an RL agent that could efficiently explore a new environment after learning in similar MDPs.
...
Anyway, thanks for reading! I hope you've managed to one-shot learn the concept of one-shot learning :) If not, I'd love to answer any questions you have!
