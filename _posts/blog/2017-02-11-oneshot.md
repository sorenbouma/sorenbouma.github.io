...
One cool thing I noticed about training on pairs is that there are combinatorialy many pairs of images to train the model on, making it hard to overfit. Say we have \\( E\\) examples each of \\( E \\) classes. Since there are \\( C \cdot E \\) images total, the total number of possible pairs is given by
...
[One-shot Imitation learning](https://arxiv.org/abs/1703.07326) is one of my favourite o.s.l papers. The goal is to have an agent learn a robust policy for solving a task from a single human demonstration of that task.This is done by: 
1. Having a neural net that from the current state and a sequence of states(the human demonstration) to the action state
...
This strikes me as a really promising path to having broadly applicable, learning based robots!
