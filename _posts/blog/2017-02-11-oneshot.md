...
This ability to rapidly learn from very little data seems like it's obviously desirable for machine learning systems to have because collecting and labelling data is expensive. 


It would be silly to have to collect lots of new data to get the pedestrian detection model in a self driving car to work in Pauanui, which has weird looking red roads:
...
It is also given \\( \hat{x} \\), the test example it has to classify. Since exactly one example in the support set has the right class, the aim is to correctly guess which \\( y \in S \\) is the same as \\(\hat{x} \\)'s class, \\(\hat{y}\\).
...
\\[ C(\hat{x}) = \underset{c}{\operatorname{argmin}} \|\| \hat{x} - x_c \|\|, x_c \in S \\]
...
As you can see, it performs worse on tasks from the validaiton set than the train set, especially for high values of N, so there must be overfitting. It would be interesting to see if 
