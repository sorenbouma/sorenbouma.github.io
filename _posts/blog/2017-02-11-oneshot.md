...
However, the siamese network needs examples of both same and different class pairs. There are \\( E\\) examples per class, so there will be  \\( {\binom {E}{2}}  \\) pairs for every class, which means there are  \\(N_{same} = {\binom {E}{2}}C  \\) possible pairs with the same class -  183,160 pairs for omniglot. Only a thousandth of the pairs have the same class, which is important because the siamese network should be given the same number of same-class and different-class pairs at each iteration.
